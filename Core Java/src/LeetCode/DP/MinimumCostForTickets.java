...
//	minCost(i) = minCost(i − 7) + costs[2]
	public static int mincostTickets(int[] days, int[] costs)
	{
        ...
        for (int i = 1; i < 366; i++) 
        {
            ...
            dp[i] = Math.min(dp[i], dp[Math.max(0, i - 1)] + costs[0]);		//for 1 Day pass
            dp[i] = Math.min(dp[i], dp[Math.max(0, i - 7)] + costs[1]);		//for 7 Day pass
            dp[i] = Math.min(dp[i], dp[Math.max(0, i - 30)] + costs[2]);	//for 30 Day pass
        }
        return dp[365];
    }
