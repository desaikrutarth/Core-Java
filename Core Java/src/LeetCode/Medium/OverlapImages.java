...
Input: A = [[1,1,0],
       ...
       B = [[0,0,0],
            ...
            [0,0,1]]
Output: 3
Explanation: We slide A to right by 1 unit and down by 1 unit. So Total there are 3 1s in overlap.
 */
public class OverlapImages
{
	public static int largestOverlap(int[][] A, int[][] B)
	{
        ...
        List<int[]> listB = new ArrayList<>(); // two lists to save pixel coordinates
        ...
        for (int r = 0; r<rows; r++)
        {
            ...
            {
                if (A[r][c] == 1)
                	listA.add(new int[]{r,c}); // save the pixel coordinates
                if (B[r][c] == 1)
                	listB.add(new int[]{r,c});
            }
        }
